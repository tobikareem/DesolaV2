...
export const Navbar = () => {
  ...
  const { navigationData, mobileTab, setMobileTab } =
    useContext(NavigationContext);
  const { toggleModal } = useContext(UIContext);

  const handleDrawer = (
    e: React.MouseEvent<
      HTMLDivElement | HTMLButtonElement | HTMLAnchorElement | SVGElement
    >
  ) => {
    if ((e.target as HTMLElement).id !== "drawer") {
  ...
  return (
    <div
      className={`fixed ${router !== "/" ? "hidden" : "flex"} ${
        !isDesktop && router === "/dashboard" ? "!flex" : ""
      }  items-center w-screen z-40 bg-transparent  left-0 backdrop-blur-[2px] `}
    >
      <div className="flex w-full justify-between items-center px-4 md:px-8 lg:px-14 xl:px-28 py-4 bg-background">
        ...
        <div
          onClick={() => setSlider((prevState) => !prevState)}
          className={`block lg:hidden ${
            router === "/dashboard" ? "text-primary-600" : "text-neutral-900"
          } text-2xl`}
        >
          <GiHamburgerMenu />
        </div>
          ...
          {Navigation?.map((item, index) => {
            return (
              <NavLink
                ...
                className={() =>
                  `font-work h-full p-3 text-base text-neutral-900 font-medium rounded-[48px] text-nowrap hover:bg-secondary-100 hover:scale-105 transition-transform duration-300 ease-in-out ${
        ...
        {isAuthenticated ? (
          ...
          <button
            ...
            className={`hidden lg:flex items-center gap-2 justify-center h-12 ${
              isSigningIn ? "w-[180px]" : "w-[140px]"
            } bg-gradient-to-b from-[#FF9040] to-[#FF6B00] text-base text-white font-medium rounded-[48px] hover:scale-105 transition-transform duration-300 ease-in-out`}
          >
            {isSigningIn ? (
              <>
                <span>Signing in...</span>
                <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white mr-4" />
              </>
            ) : (
              "Sign In"
            )}
        ...
        {!isDesktop && (
          <div
            className={`block lg:hidden w-screen h-svh ${
              slider
                ? "translate-x-0  bg-black/60"
                : "translate-x-[-110%] bg-transparent"
            } transition-all duration-200 ease-in fixed top-0 left-0 z-50`}
          >
              ...
              >
                <nav className={`flex flex-col w-full py-1 px-1.5 gap-6 `}>
                    ...
                    : Navigation?.map((item, index) => {
                        return (
                          <NavLink
                            ...
                            className={() =>
                              `font-work size-full p-3 text-base text-neutral-900 !font-medium rounded-lg text-nowrap hover:bg-secondary-100 hover:scale-105 transition-transform duration-300 ease-in-out ${
                  ...
                  <div
                    ...
                    className={`${
                      router === "/dashboard" ? "flex" : "hidden"
                    } w-fit items-center gap-3 text-primary-300 font-medium font-work text-base cursor-pointer hover:scale-105 duration-300 `}
                  >
                  ...
                  >
                    {isSigningIn ? (
                      <>
                        ...
                        <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white mx-4" />
                      </>
                    ) : (
                      "Sign In"
                    )}
